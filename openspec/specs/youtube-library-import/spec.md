...
Application-driven state changes (e.g., the user transitioning a video from `inbox → saved` via the `consumption` lifecycle) SHALL be preserved across subsequent re-imports — the upsert does not interact with consumption state beyond the NEW-row insertion path.
